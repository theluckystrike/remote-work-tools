...
Specs:
- 84 keys (75% layout, common for programmers)
- Aluminum case, CNC-machined
- RGB, wireless + USB-C wired
- ~2.5kg with stabilizers
...
Specs:
- Split ergo layout, 70+ keys each half
- Contoured key wells for wrist relief
- Onyx case (high-quality plastic)
- Wireless (2.4GHz + BT), USB wired
- 1.2kg per half
...
Programmability - Kinesis uses its own firmware (not QMK), with a learning curve. Onboard programming: plug into a PC, upload configuration file (text-based), reprogram layers, macros, key remapping. Full power available, but less extensive environment than QMK. Most users spend 2, 3 hours getting a comfortable layout.
...
Downsides:
- Expensive ($449, highest here)
- Large footprint; not portable (split design takes desk space)
- Ergonomic layout is polarizing (some users hate it; others can't type on traditional keyboards after using it)
- Onboard programming is less flexible than QMK
...
Specs:
- Split ergonomic layout, 66 keys
- Columnar/staggered layout
- Wired USB or wireless (2.4GHz via USB adapter)
- Build your own kit (requires soldering or pre-assembled option at +$35)
- ~650g total
...
- "Wireless" is misleading (not truly cordless)
- Ergonomic benefits less pronounced than Kinesis
- Larger learning curve with QMK vs Kinesis's simpler firmware
...
- Budget is primary concern ($189 is lowest entry to mechanical quality)
- You want full QMK customization
- You value wireless + upgradeable switches
- You're fine with standard layout

Setup cost - Q1 + backup switch set + stabilizer mods = ~$220 (one-time)

Choose HHKB Pro 2 if:
- You travel frequently (lightest, most portable)
- You value ultimate typing feel and build quality
- Budget for premium is available ($295+)
- You don't need deep customization

Setup cost - HHKB + carrying case = ~$320 (one-time)

Choose Kinesis Advantage 360 if:
- You have existing wrist pain or RSI
- Desk space is available (large footprint)
- You want measurable ergonomic benefit
- Budget allows ($449)

Setup cost - Kinesis 360 only = $449 (one-time, no add-ons needed)

Choose ZSA Moonlander if:
- You want ergonomics + full customization (QMK)
- You're comfortable with moderate assembly
- You want switch flexibility
- Budget is $365
...
- Best overall value: Keychron Q1 Pro ($189). Solid all-around, good for most developers.
- Best for travel/premium: HHKB Pro 2 ($295). Unmatched portability and typing feel.
