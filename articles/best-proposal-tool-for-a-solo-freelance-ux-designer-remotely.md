...
- Already have a strong portfolio to link
...
- Proposals flow directly into contracts
- Time tracking links to project estimates
...
- Best for: Initial engagements, low-commitment introductions
...
- Click heat maps (discovering hidden navigation issues)
- A/B tested 3 signup flows

Outcome:
- Reduced abandonment from 40% to 18%
...
- Design training or handoff sessions
- Post-launch support (available separately at $[rate]/hour)

Changes after this list requires a change order.
```
...
```javascript
// Zapier + PandaDoc: Auto-generate proposals from form responses

// When form submitted with client info:
// 1. Extract client name, project type, budget
// 2. Select appropriate template based on project type
// 3. Populate template with client details
// 4. Send draft for review (you check before sending)

// This reduces proposal creation from 90 minutes to 15 minutes per deal
```
